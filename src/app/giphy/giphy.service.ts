...
export class GiphyService {
  // BehaviorSubject se subscribe al valor por defecto y al nuevo valor con el next
  ...
  giphyResponse$ = combineLatest([
    ...
    switchMap(([actualPage, searchTerm, limit]) =>
      this.http.get(
        `${API_SEARCH_URL}?q=${searchTerm}&api_key=${API_KEY}&limit=${limit}&offset=${actualPage *
          limit}`
      )
    )
  );
  ...
  gifs$ = this.giphyResponse$.pipe(
    map((resp: GiphyResponse) => resp.data)
  )

  totalResults$ = this.giphyResponse$.pipe(
    map((resp: GiphyResponse) => resp.pagination.total_count)
  )
